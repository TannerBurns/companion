...
use super::types::{AtlassianError, AtlassianTokens, CloudResource, ConfluencePage, JiraIssue};
...
impl AtlassianClient {
    ...
    pub async fn start_oauth_flow(
        &self,
    ) -> Result<(AtlassianTokens, Vec<CloudResource>), AtlassianError> {
        ...
        let rx = spawn_oauth_callback_listener(REDIRECT_PORT, state)
            .await
            .map_err(|e| {
                AtlassianError::OAuth(format!("Failed to start callback listener: {}", e))
            })?;
    ...
    async fn exchange_code(
        ...
        code: &str,
        code_verifier: &str,
    ) -> Result<AtlassianTokens, AtlassianError> {
        #[derive(Deserialize)]
    ...
    async fn get_accessible_resources(
        &self,
        access_token: &str,
    ) -> Result<Vec<CloudResource>, AtlassianError> {
        #[derive(Deserialize)]
    ...
    pub async fn search_issues(
        ...
        start_at: i32,
        max_results: i32,
    ) -> Result<Vec<JiraIssue>, AtlassianError> {
        let token = self
            .access_token
            .as_ref()
        ...
        let response = self
            ...
            .query(&[
                ...
                (
                    "fields",
                    "summary,description,status,assignee,reporter,project,created,updated",
                ),
            ])
            .send()
            .await?;
