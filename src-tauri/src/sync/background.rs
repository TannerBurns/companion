...
impl BackgroundSyncService {
    ...
    pub async fn set_interval(&self, interval_minutes: u64) {
        ...
        tracing::info!("Updated sync interval to {} minutes, resetting timer", interval_minutes);
    ...
    pub fn start(&self) {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let next_sync = chrono::Utc::now().timestamp_millis() + (interval as i64 * 60 * 1000);
    ...
    async fn run_sync_cycle(
        ...
        if total_items > 0 {
            if let Some(api_key_or_client) = get_gemini_client(db.clone(), crypto.clone()).await {
                let ai_task_id = {
                    let pipeline = pipeline.lock().await;
                    pipeline.start_task(PipelineTaskType::AiSummarize, "Analyzing content with AI...".to_string()).await
                };
                ...
                match ai_pipeline.process_daily_batch(timezone_offset).await {
                    Ok(processed) => {
                        ...
                        let pipeline = pipeline.lock().await;
                        pipeline.complete_task(&ai_task_id, Some(format!("Summarized {} items", processed))).await;
                    }
                    Err(e) => {
        ...
        tracing::info!("Background sync cycle completed: {} items in {}ms", total_items, duration_ms);
    }
}
...
pub async fn get_last_sync_at(db: Arc<Database>) -> Option<i64> {
    let result: Option<(String,)> = sqlx::query_as(
        "SELECT value FROM preferences WHERE key = 'last_sync_at'"
    )
    .fetch_optional(db.pool())
    .await
    .ok()?;
