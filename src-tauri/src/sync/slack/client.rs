...
impl SlackClient {
    ...
    pub async fn test_auth(&self) -> Result<SlackAuthInfo, SlackError> {
        ...
        let team_domain = json["url"].as_str().and_then(|url| {
    ...
    async fn exchange_code(&self, code: &str) -> Result<SlackTokens, SlackError> {
        let response = self
            ...
            .form(&[
                ...
                (
                    "redirect_uri",
                    &format!("http://localhost:{}/slack/callback", REDIRECT_PORT),
                ),
            ])
            .send()
            .await?;
        ...
        let user = oauth_response
            .authed_user
            .ok_or_else(|| SlackError::OAuth("No user token".into()))?;
        let team = oauth_response
            .team
        ...
        Ok(SlackTokens {
            access_token: user
                .access_token
                .ok_or_else(|| SlackError::OAuth("No access token".into()))?,
    ...
    pub async fn get_channel_history(
        ...
        let mut params = vec![("channel", channel_id), ("limit", &limit_str)];
...
mod tests {
    ...
    fn test_with_token() {
        let client =
            SlackClient::new("client_id".into(), "secret".into()).with_token("xoxp-token".into());
    ...
    async fn test_get_channel_history_requires_auth() {
        let client = SlackClient::new("id".into(), "secret".into());
        let result = client
            .get_channel_history("C123", None, None, None, 100)
            .await;
