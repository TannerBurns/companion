...
describe('AboutSection', () => {
  ...
  it('shows update available with download button', async () => {
    mockUseUpdater.mockReturnValue({
  ...
  it('calls downloadAndInstall when download button is clicked', async () => {
    mockUseUpdater.mockReturnValue({
      state: { status: 'available', update: { version: '2.0.0' } } as never,
      checkForUpdates: mockCheckForUpdates,
      downloadAndInstall: mockDownloadAndInstall,
  ...
  it('shows restart button when update is ready', async () => {
    mockUseUpdater.mockReturnValue({
